...
public interface ScheduleService extends Runnable {
    ...
    default void run() {
        ...
        String taskId = springScheduleCron.getId() + "";
        // 获取操作redis的bean
                ...
                if (earliestTuple.isEmpty()) {
                    ...
                    if (!currentInstanceId.equals(firstInstanceId)) {
                        ...
                        if (!executeTimeMillis.equals(stringOperations.get(impeachKey))) {
                            // 检举该服务实例，检举记录保留30天
//                            log.debug("当前实例:{}检举:{}实例,任务id:{},bean:{}", currentInstanceId, firstInstanceId, taskId, beanName);
            ...
            boolean tryLock = stringOperations.setIfAbsent(lockKey, "", 5, TimeUnit.SECONDS);
            if (!tryLock) {
                // 未得到执行权
                return;
            }
            // 更新该服务实例的执行时间
//            log.debug("当前实例:{}获得执行权,任务id:{},bean:{}", currentInstanceId, taskId, beanName);
            zSetOperations.add(zsetKey, currentInstanceId, System.currentTimeMillis());
            execute();
        } catch (Exception e) {
            log.error("redis异常", e);
     ...
     * @return
     */
    default boolean validInstance(String instanceId) {
